...
class SmallSurvey(Survey):
    ...
    def __init__(
        ...
        All parameters except _kwargs are stored as attributes.
        ...
        if downlink_freq not in downlink_rate_kwargs:
            raise ValueError(
                f'Key "{downlink_freq}" is not found '
                f'in the downlink_rate_kwargs dictionary.'
            )
        self.downlink_rate = downlink_rate_kwargs.get(downlink_freq) 
        # satellite minimum elevation to link with ground station
        if downlink_freq not in min_elevation_kwargs:
            raise ValueError(
                f'Key "{downlink_freq}" is not found '
                f'in the min_elevation_kwargs dictionary.'
            )
        self.min_elevation = min_elevation_kwargs.get(downlink_freq)
        # maximum cloud coverage % for optical downlink
        if downlink_freq not in max_cloud_cover_kwargs:
            raise ValueError(
                f'Key "{downlink_freq}" is not found '
                f'in the max_cloud_cover_kwargs dictionary.'
            )
        self.max_cloud_cover = max_cloud_cover_kwargs.get(downlink_freq)
        # list of ground station coordinates 
        if downlink_freq not in ground_stations_kwargs:
            raise ValueError(
                f'Key "{downlink_freq}" is not found '
                f'in the ground_stations_kwargs dictionary.'
            )
        self.ground_stations = ground_stations_kwargs.get(downlink_freq)
                ...
                if tile.occultation > 0:  # If the tile is currently occulted
                    ...
                    if other_obs_time > min_other_obs_time:
                        ...
                        if power_deficit_time != None:
                            ...
                            # print(
                            #     f"The satellite has operated on battery "
...
class WideSurvey(Survey):
    ...
    def __init__(
        ...
        downlink_rate_kwargs : dict
            Dictionary specifying the data downlink rate (in GBps) for each frequency band.

        min_elevation_kwargs : dict
            Dictionary defining the minimum satellite elevation angle (in degrees) required 
            to establish contact with ground stations for each frequency band.
        ...
        ground_stations_kwargs : dict
            Dictionary containing lists of ground station coordinates (latitude, longitude) 
        ...
        min_elevation : float
            The minimum satellite elevation, in degrees, to establish contact with the ground stations 
            of the chosen frequency band.

        max_cloud_cover : float or None
            The maximum percentage of cloud coverage that still allows for optical downlinking. 
            `None` for non-optical frequency bands.

        ground_stations : list of tuples
            A list of ground station coordinates (latitude, longitude) available for the chosen frequency band.
            
        tile_rotation : float
            Tile rotation angle in degrees counter-clockwise.

        tile_sequence : list of tuples
            A list of tile center coordinates (RA, Dec) in degrees, ordered in vertical snake scan sequence.

        tile_shapes : list of shapely.geometry.MultiPolygon
            A list of tiles as MultiPolygon objects ordered in vertical snake scan sequence.

        tile_bins : list of lists
            A list of bins where each bin is a list of tuples representing tile centers (RA, Dec) in degrees.
            The bins are ordered by ascending RA, and within each bin, the tiles are sorted by descending Dec.
        ...
        self.data = data # amount of data (in GB) obtained per subexposure readout
        self.data_compression_fac = data_compression_fac # onboard data compression factor
        self.internal_data_rate = internal_data_rate # internal data generation rate (in GBps)
        self.onboard_data_cap = onboard_data_cap # onboard data capacity (in GB)
        ...
        if downlink_freq not in downlink_rate_kwargs:
            raise ValueError(
                f'Key "{downlink_freq}" is not found in the downlink_rate_kwargs dictionary.'
            )
        self.downlink_rate = downlink_rate_kwargs.get(downlink_freq) 
        # satellite minimum elevation to link with ground station
        if downlink_freq not in min_elevation_kwargs:
            raise ValueError(
                f'Key "{downlink_freq}" is not found in the min_elevation_kwargs dictionary.'
            )
        self.min_elevation = min_elevation_kwargs.get(downlink_freq)
        # maximum cloud coverage % for optical downlink
        if downlink_freq not in max_cloud_cover_kwargs:
            raise ValueError(
                f'Key "{downlink_freq}" is not found in the max_cloud_cover_kwargs dictionary.'
            )
        self.max_cloud_cover = max_cloud_cover_kwargs.get(downlink_freq)
        # list of ground station coordinates 
        if downlink_freq not in ground_stations_kwargs:
            raise ValueError(
                f'Key "{downlink_freq}" is not found in the ground_stations_kwargs dictionary.'
            )
        self.ground_stations = ground_stations_kwargs.get(downlink_freq)

        # Power
        self.solar_panel_normals = solar_panel_normals # an array of each solar panel normal vectors
        self.solar_panel_areas = solar_panel_areas # an array of each solar panel area (in m²)
        self.solar_panel_eff = solar_panel_eff # end-of-life efficiency 
        self.battery_power = battery_power # available batter in an hour (in W-hr)
        self.power_load = power_load # average power consumption (in W)
        ...
        self.tile_target_bins = [] # stores a list of tile Target objects in bins

    
    def __repr__(self):
        return f"<WideSurvey object: exposure={self.exposure} s, downlink_freq='{self.downlink_freq}'>"

    
    def tile(
        self, rotation_angle=0, min_intersect_ratio=0.5, bin_size=2, dec_cutoff=None, cut_below=True, 
        tile_overlap=params.TILE_OVERLAP, l=params.DETECTOR_LENGTH, w=params.DETECTOR_WIDTH,
        g=params.DETECTOR_ARRAY_GAP
    ):
        """
        Tiles the survey footprint, where each tile represents the coverage of a single telescope pointing.
        The footprint boundary can be modified using the `dec_cutoff` parameter if needed.
        The tiles are grouped based on their Right Ascension (RA) coordinates into bins. 
        Within each bin, the tiles are sorted by Declination (DEC) in descending order.
        ...
        trimmed_boundary_coords = cut_footprint(self.boundary_coords, dec_cutoff, cut_below)
        # Order the tiles in snake scan sequence
        tile_centers, tile_shapes = snake_scan_tiling(trimmed_boundary_coords, rotation_angle, min_intersect_ratio, 
                                                      tile_overlap, l, w, g)
        self.tile_sequence = tile_centers
        self.tile_shapes = tile_shapes
        ...
        return Tiling(self.boundary_coords, self.tile_rotation, self.tile_sequence, self.tile_shapes, self.tile_bins)

    
    def observe(self, start_day=None, num_days_ahead=60, min_other_obs_time=60, data_threshold=100, 
        ...
        The function determines the most efficient observing sequence to maximize time spent on the survey 
        during the observing season, aiming to observe as many tiles as possible within the available timeframe.

        The function calculates and outputs the survey's observing efficiency and provides a breakdown 
        of total clock time allocations in a dictionary, including:
            - Time spent on other surveys
            - Idle time
            - Time spent on each operation tasks
        Note: Downlinks are assumed to occur simultaneously with other operational tasks, except for slews.
        
        Parameters
        ----------
        start_day : astropy.time.Time, optional
            The day to start observing the survey. 
            The default day is `num_days_ahead` of the day corresponding to when the smallest RA tile 
            in the survey is in the anti-Sun direction.

        num_days_ahead : int, default=60
            The number of days before the default `start_day` to start the observation. Only applicable 
            when `start_day` is None. The default is set 60 days ahead. 

        min_other_obs_time : float, default=60
            The minimum continuous observation time (in seconds) required for the other surveys.
        ...
        -------
        obs_results : dict of dict
            A nested dictionary that compiles the observation results accumulated after the completion of each bin.
            Each bin is represented as a key in the outer dictionary, with the value being a dictionary that 
            documents the current observation results with the following keys:
                - "obs_efficiency": float
                    The observing efficiency (%).
                - "total_clock_time": float
                    The total clock time (seconds).
                ...
                - "readout_time": float
                    Total data readout time (seconds).
            The "final" key contains the final observation results after all bins are completed.
        
        data_tracker : dict
            A dictionary structured as {"time":[], "data_onboard":[]} that tracks the amount of 
            data onboard (in GB) 
            at different times throughout the observation. 
                - "time": list of Astropy Time objects 
                    The timestamps when the data onboard is recorded.
                - "data_onboard": list of floats 
                    The amount of data onboard (in GB) at each timestamp.
        ...
        obs_sequence : list
            A list of tile Target objects (each with updated `status` attribute) ordered in the observing sequence.
        """
        # Stores the observation results after each bin is completed, and the final result of the survey
        obs_results = {}
        ...
        # Define the range of slew times (seconds) allowed for slewing within the survey field
        # the lower limit prevents slewing to another tile with similar viewing efficiency (at similar location)
        ...
        if start_day is None:
            # The default version uses the day that is `num_days_ahead` of the day corresponding to 
            # when the smallest RA tile in the survey is in the anti-Sun direction.
        ...
        start_time = self.tile_target_bins[0][-1].get_visibility_windows(
            start_day, start_day + period * u.min, 60, self.satellite.earth_satellite).get_start_time()
        Now = start_time
        ...
        # Monitors the continuous duration during which the satellite is in power deficit.
        power_deficit_time = None
        # Tracks the amount of data onboard throughout the observation
        data_tracker = {"time":[], "data_onboard":[]}
        # Stores tiles in the observing sequence 
        ...
        for i, bin in enumerate(bins):
            ...
            while bin:
                ...
                if Now >= end_search:
                    ...
                    runtime = timeit.default_timer() - search_code_start
                    # print(
                    #     f"There are {downlink_windows.num} downlink windows from {start_search} to {end_search}"
                    # )
                    # print("runtime:", print_time(runtime))
                ...
                if tile_num == 0: # first tile of the survey
                    ...
                    CVZ = InertialTarget(*find_CVZcenter(self.satellite.earth_satellite, Now))
                    slew = field_slew(
                        CVZ, tile, self.satellite.earth_satellite, Now, self.satellite.telescope_boresight, 
                        self.satellite.solar_array, self.solar_panel_normals, self.solar_panel_areas, 
                        self.solar_panel_eff, self.battery_power, self.power_load, 
                        self.satellite.slew_angle_data, self.satellite.slew_time_data
                    )
                    ...
                    adjacent_tile = bin[0] if top else bin[-1]  # Could be a new tile or the previous tile
                    ...
                    if adjacent_tile != prev_tile:  # Avoid re-calculating previous tile's visibility window
                        adjacent_slew = tile_slew(
                            prev_tile, adjacent_tile, self.tile_rotation, self.satellite.telescope_boresight, 
                            self.satellite.earth_satellite, Now, self.solar_panel_normals, self.solar_panel_areas, 
                            self.solar_panel_eff, self.battery_power, self.power_load, 
                            self.satellite.slew_angle_data, self.satellite.slew_time_data
                        )
                        ...
                        adjacent_tile.occultation = find_occultation(
                            adjacent_tile, Now, adjacent_slew + self.settle + self.acquire_guide,
                            self.subexposure, period, self.satellite.earth_satellite
                        )
                    ...
                    # whether because occultation occurred while imaging the previous tile 
                    ...
                    if adjacent_tile == prev_tile or adjacent_tile.occultation > 0:
                        ...
                        opposite_slew = tile_slew(
                            prev_tile, opposite_tile, self.tile_rotation, self.satellite.telescope_boresight, 
                            self.satellite.earth_satellite, Now, self.solar_panel_normals, self.solar_panel_areas, 
                            self.solar_panel_eff, self.battery_power, self.power_load, 
                            self.satellite.slew_angle_data, self.satellite.slew_time_data
                        )
                        ...
                        opposite_tile.occultation = find_occultation(
                            opposite_tile, Now, opposite_slew + self.settle + self.acquire_guide,
                            self.subexposure, period, self.satellite.earth_satellite
                        )

                        # Select the opposite tile if it is currently visible
                        # or if the slew time is in the desired range
                        if (opposite_tile.occultation == 0) or (min_slew_time < opposite_slew < max_slew_time):
                ...
                if tile.occultation > 0:  # If the tile is currently occulted
                    ...
                    CVZ = InertialTarget(*find_CVZcenter(self.satellite.earth_satellite, Now))
                    slew_to_CVZ = field_slew(
                        prev_tile, CVZ, self.satellite.earth_satellite, Now, self.satellite.telescope_boresight, 
                        self.satellite.solar_array, self.solar_panel_normals, self.solar_panel_areas, 
                        self.solar_panel_eff, self.battery_power, self.power_load, 
                        self.satellite.slew_angle_data, self.satellite.slew_time_data
                    )
                    ...
                    # Find the observation time available for the other survey
                    # Note: Assume the slew back time is the same as 'slew_to_CVZ'
                    other_obs_time = tile.occultation - (slew_to_CVZ + self.settle + self.acquire_guide) * 2
                    ...
                    if other_obs_time > min_other_obs_time:
                        ...
                        if power_deficit_time != None:
                            ...
                            if power_deficit_time > obs_time_limit:
                                print()
                                print("\033[1mError:\033[0m The battery is severely depleted.")
                                return obs_results, data_tracker, obs_sequence

                        # Check if pointing at the `CVZ` results in power deficit
                        if CVZ.obs_time_limit != np.inf:
                            # Initiate the tracker to monitor the continuous duration of power deficit
                            if power_deficit_time is None:
                                power_deficit_time = 0
                                # record the maximum observation time allowed
                                obs_time_limit = CVZ.obs_time_limit
                            ...
                            # )
                            # print(
                            #     f"Can observe for up to {print_time(CVZ.obs_time_limit)} "
                            #     "before battery depletion."
                            # )
                        else:
                            # Terminate the power deficit time tracker
                        ...
                        # Note: Avoid downlinking while slewing
                        try:
                            check_period = self.settle + self.acquire_guide + other_obs_time
                            Data_Onboard = downlinking(
                                check_period, Now, Data_Onboard, data_tracker, downlink_windows, data_threshold,
                                self.onboard_data_cap, self.downlink_rate, min_downlink_time=60
                            )
                        except ValueError as e:
                            print()
                            print(f"\033[1mError:\033[0m {e}")
                        ...
                        # Document the time spent on the other survey
                        other_survey = slew_to_CVZ + self.settle + self.acquire_guide + other_obs_time
                        other_survey_segments.append(other_survey)
                        # print("Other survey time:", print_time(other_survey))
                        ...
                        # Track the time elapsed during periods of power deficit
                        if power_deficit_time != None:
                            power_deficit_time += self.settle + self.acquire_guide + other_obs_time
                            # print(
                            #     f"The satellite has operated on battery "
                            #     f"for {print_time(power_deficit_time)} continuously."
                            # )
                    ...
                    # Or else count the occultation time as idle time (and not slew to other surveys)
                    else:
                        tile.status = "idle"
                        idle = tile.occultation
                        # Subtract the time to slew back to the survey from idle time (if applicable)
                        if slew != 0:
                            idle -= slew + self.settle + self.acquire_guide
                        idle_time_segments.append(idle)
                        # print("Add idle time:", print_time(idle))
                ...
                if slew != 0:
                    ...
                    # print("slew:", print_time(slew))
                    # print(f"tile {tile.index} is at ({tile.ra:.3f}°, {tile.dec:.3f}°)")
                    # print("settle + guide star acquisition:", print_time(self.settle + self.acquire_guide))
                    # print("Time Elapsed:", print_time((Now - start_time).sec))
                ...
                while tile.subexposure_count < self.subexposure_num:
                    # Check for occultation
                    tile.occultation = find_occultation(
                        tile, Now, 0, self.subexposure, period, self.satellite.earth_satellite
                    )
                    # Stop imaging and reselect tile if occultation occurs
                    if tile.occultation > 0:
                        imaged = False
                    ...
                    imaged = True  # Track if the tile is imaged at least once in this cycle
                    ...
                    # Data generated from image readout (accounts for data compression if applicable)
                    Data_Onboard += self.data / self.data_compression_fac
                
                    # Check for downlink opportunities during imaging
                    try:
                        check_period = self.subexposure + self.dither / self.subexposure_num + self.readout
                        Data_Onboard = downlinking(
                            check_period, Now, Data_Onboard, data_tracker, downlink_windows, data_threshold,
                            self.onboard_data_cap, self.downlink_rate, min_downlink_time=60
                        )
                    ...
                    # Update time
                    Now += (self.subexposure + self.dither / self.subexposure_num + self.readout) * u.s
                    # print("imaging + dither per subexposure:", 
                    #       self.subexposure + self.dither / self.subexposure_num + self.readout, "s")

                    # Track the time elapsed during periods of power deficit
                    if power_deficit_time != None:
                        power_deficit_time += self.subexposure + self.dither / self.subexposure_num + self.readout
                        # print(
                        #     f"The satellite has operated on battery "
                        #     f"for {print_time(power_deficit_time)} continuously."
                        # )
                ...
                if imaged:
                    # Append a copy of the tile so status changes won't affect the copy in the list
                    obs_sequence.append(copy.deepcopy(tile))
                
                # Account for internally generated data
                Data_Onboard += (Now - prev_internal_data_check).sec * self.internal_data_rate
                prev_internal_data_check = Now
            ...
            bin_clock_time = (Now - prev_bin_ends).sec  # Time spent on this bin
            Total_Clock_Time = (Now - start_time).sec  # Total clock time in seconds
            Other_Survey_Time = sum(other_survey_segments)  # Total time spent on other surveys
            Idle_Time = sum(idle_time_segments)  # Total idle time in seconds
            This_Survey_Time = Total_Clock_Time - Other_Survey_Time  # Total survey time (including idle)
            obs_efficiency = (Exposure / This_Survey_Time) * 100  # Observing efficiency (%)
            ...
            print(f"\033[1mCumulative observing efficiency: {obs_efficiency:.2f} %\033[0m")
